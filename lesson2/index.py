    ...
    for job in list:
        ...
        if vacancy is not None:
            ...
            if price is not None:
                ...
                if len(subs) > 0:
                    job_data['price_from'] = subs[0][0].replace('\xa0', ' ')
                    job_data['currency'] = subs[0][1]
                else:
                    subs = pattern2.findall(price)
                    if len(subs) > 0:
                        job_data['price_from'] = subs[0][0].replace('\xa0', ' ')
                        job_data['price_to'] = subs[0][1].replace('\xa0', ' ')
                        job_data['currency'] = subs[0][2]
                    else:
                        subs = pattern3.findall(price)
                        if len(subs) > 0:
                            job_data['price_to'] = subs[0][0].replace('\xa0', ' ')
                            job_data['currency'] = subs[0][1]
                        else:
...
while (page_count > 0) and (len(superjob_sub_link) > 0):
    ...
    list = parsed_hh.find("div", {"class": "iJCa5 _1JhPh _2gFpt _1znz6 _1LlO2 _2nteL"}). \
        ...
        find('div', {'class': '_3zucV'}).find('div', {'style': 'display:block'}).findChildren('div', recursive=False)
    for job in list:
        job_data = {}
        ...
        if vacancy is not None:
            ...
            if len(price) == 2:
                job_data['price_from'] = price[0]
                job_data['currency'] = price[1]
            elif len(price) == 4:
                job_data['price_from'] = price[0]
                job_data['price_to'] = price[2]
                job_data['currency'] = price[3]

            job_data['source'] = superjob_link
            jobs.append(job_data)

    superjob_sub_link = parsed_hh.find('a', {'class': 'icMQ_ _1_Cht _3ze9n f-test-button-dalshe f-test-link-dalshe'})
